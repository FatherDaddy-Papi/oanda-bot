...
- **Clenow V2 (correlation-capped, `clenow_v2.py`):** ❌ FAIL. Parameter-free group cap (max 1 instrument per a-priori asset class) to remove the concentration. In-sample Sharpe survived (ann +1.24, DSR 0.969) but **PBO rose 0.398 → 0.600 and split-half diff 0.07 → 1.02**. The frozen version's robustness metrics WERE the concentration — diversify honestly (as Gate 11 requires) and the edge becomes unreliable. By the pre-registered rule (PBO ≥ 0.50 fails even when DSR clears), V2 does not earn an OOS freeze. **This definitively closes the Clenow line: no robust deployable edge in either concentrated or diversified form.**
- No strategy holds a clean Gate 2 pass. Clenow **SHELVED** (both v1 and V2 exhausted).
